...
func ReadObjectType(hash string) (objecttype.ObjectType, error) {
	...
	for buf[0] != 0 {
		if n != 1 || err != nil {
			return objecttype.Unknown, errors.New("invalid object format")
		}
	...
	if len(header) != 2 {
		return objecttype.Unknown, errors.New("invalid object header")
	}
	...
	default:
		return objecttype.Unknown, errors.New("invalid object type")
	}
}
...
func ReadObject(hash string) (objecttype.ObjectType, []byte, error) {
	...
	if nullPos <= 0 {
		return objecttype.Unknown, nil, errors.New("invalid object format")
	}
	header := strings.Split(string(blob[:nullPos]), " ")
	content := blob[nullPos+1:]

	if len(header) != 2 {
		return objecttype.Unknown, nil, errors.New("invalid object header")
	}
	length, err := strconv.Atoi(header[1])
	if err != nil {
		return objecttype.Unknown, nil, errors.New("invalid object header")
	} else if length != len(content) {
		return objecttype.Unknown, nil, errors.New("object length mismatch")
	}
